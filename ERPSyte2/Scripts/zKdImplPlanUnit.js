(function (window, undefined) {
  ...
  function openImplPlanItem(event) {
    try {
      var cSN = this.getAttribute("data-SN");
      var cID = this.getAttribute("data-ID");
      var cLogin = document.getElementById("ftUserDataLogin").value;

      $('<form action="ImplPlanItem.html" method="GET"/>')
        .append($('<input type="hidden" name="SN" value="' + cSN + '">'))
        .append($('<input type="hidden" name="ID" value=' + cID + '>'))
  ...
  function returnData(data) {
    try {
      ...
      $(".aImplPlanCreate").click(openImplPlanItem);
      $(".aImplPlanUpdate").click(openImplPlanItem);
      $("#tabs").tabs("option", "active", 0);
